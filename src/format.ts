...
// Intl formatters are expensive to construct; cache per timezone. An invalid
// timezone falls back to UTC rendering rather than throwing in the read path
// (setTimezone validates, but the guard keeps recall alive if bad data gets in).
const dateFmtCache = new Map<string, Intl.DateTimeFormat | null>();
const timeFmtCache = new Map<string, Intl.DateTimeFormat | null>();

function cachedFmt(cache: Map<string, Intl.DateTimeFormat | null>, tz: string, opts: Intl.DateTimeFormatOptions, locale: string): Intl.DateTimeFormat | null {
  if (!cache.has(tz)) {
    try { cache.set(tz, new Intl.DateTimeFormat(locale, { timeZone: tz, ...opts })); }
    catch { cache.set(tz, null); }
  }
  return cache.get(tz)!;
}

/** YYYY-MM-DD — in the user's timezone when known, UTC otherwise. */
function isoDate(ts: number, tz?: string): string {
  const f = tz ? cachedFmt(dateFmtCache, tz, { year: 'numeric', month: '2-digit', day: '2-digit' }, 'en-CA') : null;
  return f ? f.format(ts) : new Date(ts).toISOString().slice(0, 10);
}

/** HH:MM in the user's timezone. */
function localHM(ts: number, tz: string): string | null {
  const f = cachedFmt(timeFmtCache, tz, { hour: '2-digit', minute: '2-digit', hour12: false }, 'en-GB');
  return f ? f.format(ts) : null;
}

/**
 * Label for an observation timestamp. With a known user timezone the last two
 * calendar days render as "today 14:30" / "yesterday 23:05" — real local days,
 * so the agent never confabulates a "yesterday". Without one, the last ~48h
 * render as timezone-free durations ("26h ago"), because "yesterday" computed
 * in server time can be wrong by a day at the boundaries. Beyond that, dates
 * (local when tz is known) with a day count for the first two weeks.
 * Render-time only — dates inside stored summary text stay absolute.
 */
export function timeLabel(ts: number, now: number, tz?: string): string {
  const diff = now - ts;
  if (diff < 60_000) return 'just now';
  if (diff < HOUR_MS) return `${Math.floor(diff / 60_000)}m ago`;
  const hm = tz ? localHM(ts, tz) : null;
  if (hm) {
    const day = isoDate(ts, tz);
    if (day === isoDate(now, tz)) return `today ${hm}`;
    if (day === isoDate(now - DAY_MS, tz)) return `yesterday ${hm}`;
  } else if (diff < 48 * HOUR_MS) {
    return `${Math.floor(diff / HOUR_MS)}h ago`;
  }
  const days = Math.floor(diff / DAY_MS);
  if (days < 14) return `${isoDate(ts, tz)} · ${days}d ago`;
  return isoDate(ts, tz);
}
...
export function formatConfidenceMeta(
  ...
  const latestDate = isoDate(conf.latest, tz);
...
function matchLabel(m: ObservationMatch, now: number, tz?: string): string {
  if (m.kind === 'summary') {
    const start = m.startTimestamp ?? m.timestamp;
    const range = isoDate(start, tz) === isoDate(m.timestamp, tz) ? isoDate(m.timestamp, tz) : `${isoDate(start, tz)}–${isoDate(m.timestamp, tz)}`;
    const backing = m.obsCount ? ` · ${m.obsCount} obs` : '';
    return `[${range} · summary tier ${m.tier ?? 0}${backing}]`;
  }
  return `[${timeLabel(m.timestamp, now, tz)}]`;
}

/** Raw recall results — numbered, dated, best first. Observations are receipts, summaries are arcs. Agent synthesizes. */
export function formatRecall(matches: ObservationMatch[], now: number = Date.now(), tz?: string): string {
  if (matches.length === 0) return 'No relevant memories found.';
  return matches.map((m, i) => `[${i + 1}] ${matchLabel(m, now, tz)} ${m.text}`).join('\n\n');
}
 ...
 */
export function formatRecentNotes(obs: ObservationRow[], capChars = 6000, now: number = Date.now(), tz?: string): string {
  if (obs.length === 0) return '';
  const lines: string[] = [];
  let used = 0;
  for (const o of obs) {
    const line = `- [${timeLabel(o.timestamp, now, tz)}] ${o.text}`;
...
export function formatModelView(
  ...
  parts.push(formatConfidenceMeta(model.name, conf, now, tz));
  ...
  for (const s of [...summaries].sort((a, b) => a.startTimestamp - b.startTimestamp || b.tier - a.tier)) {
    ...
    const ended = now - s.endTimestamp < 48 * HOUR_MS ? ` · ended ${timeLabel(s.endTimestamp, now, tz)}` : '';
    const covered = s.covered ? ' · finer view of a period summarized above' : '';
    parts.push(`\n[tier ${s.tier} · ${backing} · ${isoDate(s.startTimestamp, tz)}–${isoDate(s.endTimestamp, tz)}${ended}${covered}]\n${s.text}`);
  }
  if (recentObs.length > 0) {
    const ordered = [...recentObs].sort((a, b) => a.timestamp - b.timestamp); // oldest→newest
    parts.push('\nRecent notes (verbatim):');
    for (const o of ordered) parts.push(`- [${timeLabel(o.timestamp, now, tz)}${o.covered ? ' · also summarized above' : ''}] ${o.text}`);
  }
  return parts.join('\n');
}

/** Observation-RAG receipts block for load_memory. */
export function formatReceipts(matches: ObservationMatch[], now: number = Date.now(), tz?: string): string {
  if (matches.length === 0) return '';
  const lines = matches.map(m => `- ${matchLabel(m, now, tz)} ${m.text}`);
