...
Recurrence is compressed as a count, not a characterization: "bad sleep on 4 of the 6 nights noted (Aug 12–17)", never "a stretch of poor sleep". Rollups may sum counts only from explicit counts in their inputs — a vague quantifier below stays vague above. Counting is a noticing done at synthesis time, when the whole batch is in front of the model; counting from raw context later is what fails.[^counting]
...
[^counting]: **Counting as retrieval — rejected (Aug 2026).** "How many bad nights this week?" fails when answered from memory: top-K similarity recall returns a sample, not a complete set, and near-identical recurring events blur together, so the model estimates and reports the estimate with a count's confidence. The retrieval-side fix — a windowed, exhaustive listing (all of a model's observations in a date range, RAG- or keyword-filtered) — was shaped and then shelved: its semantics are search, not recall, and a second retrieval tool beside `recall` would confuse agents. Counting landed on the write side instead: the summarizer states counts at synthesis time (it is the one moment the complete batch is in front of a model), rollups sum only explicit counts, and the reading agent must enumerate dated notes before any streak claim — always "N of the M noted": the count is of the record, not of reality, and a gap in the notes is a gap, not evidence either way. Streaks in the recent past are counted from the verbatim tail, which the view already carries.
