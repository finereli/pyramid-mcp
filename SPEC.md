...
A portable, agent-authored memory system exposed as a remote MCP server on Cloudflare. It packages the memory architecture proven inside Glopus - agent-authored multi-tag observations, a per-model summary pyramid, and receipt-recovery recall - so any MCP-capable agent backend can plug into it.
...
summaries         id, model_id, tier, text, start_timestamp, end_timestamp, source_count, is_dirty (unused), embedding
...
- A **summary** is one node of a model's pyramid. `tier` is **depth**: 0 = built from observations, 1 = built from tier-0 summaries, and so on.[^tiername] `summary_sources` records exactly which observations (tier 0) or summaries (tier N+1) it was built from. A summary without sources is invalid and is deleted on DO start.[^legacy]
...
Brute-force cosine over observations and summaries, blended with a 30-day-half-life recency penalty (summaries use their end timestamp). Results labeled: observations `[date]`, summaries `[start–end · summary tier T]`. Observations are receipts, summaries are arcs. The agent synthesizes; recall returns raw.
...
Survivors of the Aug 22, 2026 design review (what remained after the roads not taken below were re-litigated and re-rejected):
...
- **Relative time at render** — labels written by the formatter (recent notes, tail, summary ranges) gain relative forms: the last ~48h as "today 14:30" / "yesterday", older entries ISO plus "(Nd ago)" where useful. Targets the observed date/time-confabulation failure mode directly. Dates *inside* summary prose stay absolute — they are baked at synthesis and immutable, and a relative date in stored text goes stale by morning.
- **Transitive confidence labels** — labels today show immediate children only (a tier-2 reads "5 tier-1 summaries"), so the raw-observation count vanishes exactly where compression is deepest. Add the transitive observation count and the compression ratio (source chars → summary chars), computed once at insert via provenance, e.g. `[tier 2 · 47 obs · 61K→1.0K chars]` — plus a legend nudge: the higher the compression behind a claim, the more it deserves a `recall` check before being stated as fact. Models already calibrate on source counts intuitively; this gives them the real number.
- **Recall-struggle nudge** — in the recall tool description and instructions: when a fact took real effort to find (absent from recall, needed several queries, had to be re-derived or looked up outside), record it as a fresh observation once confirmed. Turns retrieval failures into the signal that fixes them — no logging machinery, and consistent with reality-as-writeback.
...
[^symbolic]: **Symbolic/logical compression of stored text — experimented, dropped.** Math and logic notation (which models read natively) compressed logical material up to ~10x but did almost nothing for human/psychological material — most of what this system stores. Context is cheap and getting cheaper; the complexity wasn't worth it. (Per the stats, the verbatim tail dominates view size anyway, and it must stay verbatim.)
